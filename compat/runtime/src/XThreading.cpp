...
public:
    HRESULT WINAPI QueryInterface( REFIID iid, void **out )
    {
        ...
        auto result = [&](HRESULT hr) {
            static std::atomic<bool> known_logged[3]{};
            static std::atomic<unsigned> other_calls{0};
            unsigned known = iid == __uuidof(IUnknown) ? 0 : iid == __uuidof(IAgileObject) ? 1 : iid == __uuidof(IXThreadingImpl) ? 2 : 3;
            bool log = SUCCEEDED(hr) && known < 3 ? !known_logged[known].exchange(true) : other_calls.fetch_add(1) < 32;
            if(log)std::fprintf(stderr,"xodus-taskqueue: QueryInterface iid=%08lx-%04x-%04x-%02x%02x-%02x%02x%02x%02x%02x%02x result=%08lx\n",
        ...
        if (!out) return result(E_POINTER);
        *out = nullptr;
    ...
    HRESULT WINAPI XTaskQueueCreate( XTaskQueueDispatchMode workDispatchMode, XTaskQueueDispatchMode completionDispatchMode, XTaskQueueHandle *queue ) override
    {
        ...
        HRESULT hr=::XTaskQueueCreate( workDispatchMode, completionDispatchMode, queue );
        std::fprintf(stderr,"xodus-taskqueue: XTaskQueueCreate work=%u completion=%u result=%08lx handle=%p\n",(unsigned)workDispatchMode,(unsigned)completionDispatchMode,static_cast<ULONG>(hr),queue ? *queue : nullptr);
        return hr;
    }
...
private:
    inline static thread_local BOOLEAN isTimeSensitiveThread{ false };
